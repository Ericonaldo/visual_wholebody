...
# create table asset
table_dims = gymapi.Vec3(0.3, 0.5, 0.3)
...
default_dof_pos[:5] = franka_mids[:5]
# grippers open
default_dof_pos[5:] = franka_upper_limits[5:]
...
franka_hand_index = franka_link_dict["wx250/gripper_link"]
...
for i in range(num_envs):
    ...
    hand_handle = gym.find_actor_rigid_body_handle(env, franka_handle, "wx250/gripper_link")
    hand_pose = gym.get_rigid_transform(env, hand_handle)
    init_pos_list.append([hand_pose.p.x, hand_pose.p.y, hand_pose.p.z])
    init_rot_list.append([hand_pose.r.x, hand_pose.r.y, hand_pose.r.z, hand_pose.r.w])

    # get global index of hand in rigid body state tensor
    hand_idx = gym.find_actor_rigid_body_index(env, franka_handle, "wx250/gripper_link", gymapi.DOMAIN_SIM)
...
while not gym.query_viewer_has_closed(viewer):
    ...
    gripper_sep = dof_pos[:, 5] + dof_pos[:, 6]
