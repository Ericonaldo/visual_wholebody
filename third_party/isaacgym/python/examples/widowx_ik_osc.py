...
# Load franka asset
asset_root = "../../assets"
franka_asset_file = "urdf/wx250s/urdf/wx250s.urdf"
...
default_dof_pos = torch.tensor(franka_mids[:6], device='cuda:0')
...
if controller == 'osc':
    ...
    franka_dof_props["stiffness"][:6].fill(kp)
    franka_dof_props["damping"][:6].fill(kv)
...
j_eef = jacobian[:, hand_index - 1, :6, :6]
...
dof_vel = dof_states[:, 1].view(num_envs, 8, 1)[:, :6]
dof_pos = dof_states[:, 0].view(num_envs, 8, 1)[:, :6]
...
while not gym.query_viewer_has_closed(viewer):
    ...
    if controller == 'ik':
        ...
        u = (torch.transpose(j_eef, 1, 2) @ m_eef @ (kp * dpose - kv * hand_vel)[:, :5].unsqueeze(-1)).view(num_envs, 6)
        # j_eef_inv = m_eef @ j_eef @ m_inv
        # u_null = 10 * -dof_vel + 25 * (
        #     (default_dof_pos.view(1, -1, 1) - dof_pos + np.pi) % (2 * np.pi) - np.pi)
        # u_null = mm @ u_null
        # u += ((torch.eye(5, device='cuda:0').unsqueeze(0) - torch.transpose(j_eef, 1, 2) @ j_eef_inv) @ u_null).squeeze(-1)

    if controller == "osc" and not asset_options.disable_gravity:
        ...
        g_torque = (torch.transpose(jacobian[:, :, :, :6], 2, 3) @ g_force).squeeze(-1)           # new shape is (n_envs, n_bodies, n_links)
    ...
    for i in range(4):
        ...
        if controller == 'ik':
            ...
            # pos = torch.cat([delta_pos, torch.zeros(num_envs, 2, device='cuda:0')], dim=-1) + dof_states[:, 0].view(num_envs, 8)
