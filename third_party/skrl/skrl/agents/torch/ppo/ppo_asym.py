...
class PPOAsym(Agent):
    def __init__(self,
                 ...
                 observation_space: Optional[Union[int, Tuple[int], gym.Space, gymnasium.Space]] = None, # Async space for policy func
                 state_space: Optional[Union[int, Tuple[int], gym.Space, gymnasium.Space]] = None, # Async space for value func
    ...
    def init(self, trainer_cfg: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.memory is not None:
            ...
            self._tensors_names = ["student_obs", "teacher_obs", "actions", "log_prob", "values", "returns", "advantages"]
    ...
    def act(self, states: torch.Tensor, timestep: int, timesteps: int) -> torch.Tensor:
        ...
        if timestep < self._random_timesteps:
            return self.policy.random_act({"states": states["states"]}, role="policy")

        # sample stochastic actions
        actions, log_prob, outputs = self.policy.act({"states": states["states"]}, role="policy")
        self._current_log_prob = log_prob

        return actions, log_prob, outputs

    def record_transition(self, states: dict, actions: torch.Tensor, rewards: torch.Tensor, next_states: dict, terminated: torch.Tensor, truncated: torch.Tensor, infos: Any, timestep: int, timesteps: int) -> None:
        super().record_transition(states=states, actions=actions, rewards=rewards, next_states=next_states,
                                  terminated=terminated, truncated=truncated, infos=infos, timestep=timestep,
                                  timesteps=timesteps)
        
        teacher_obs, student_obs = states["obs"], states["states"]
        
        if self.memory is not None:
            self._current_next_states = next_states["obs"]
            ...
            # compute values
            values, _, _ = self.value.act({"obs": self._state_preprocessor(teacher_obs)}, role="value")
            ...
            self.memory.add_samples(student_obs=student_obs, teacher_obs=teacher_obs, actions=actions, rewards=rewards, next_states=next_states,
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        with torch.no_grad():
            ...
            last_values, _, _ = self.value.act({"obs": self._state_preprocessor(self._current_next_states.float())}, role="value")
        ...
        for epoch in range(self._learning_epochs):
            ...
            # mini-batches loop
            for student_obs, teacher_obs, sampled_actions, sampled_log_prob, sampled_values, sampled_returns, sampled_advantages in sampled_batches:
                
                # try:
                _, next_log_prob, _ = self.policy.act({"states": student_obs, "taken_actions": sampled_actions}, role="policy")
                # except:
